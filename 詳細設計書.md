...
## 詳細設計仕様
* controller.c：メイン関数と、処理の流れを示す関数を扱うファイル。
...
- model.c
  ...
  - check_stone_input()
    * 書式
      ||型|変数名|概要|
      |:---|:---|:---|:---|
      |引数|char*|const input|石を置く場所の入力内容。|
      |戻り値|int|element_num|その場所の要素番号、またはFUNCTION_FALSE。|
    * 仕様説明  
...
- file.c
  - file_existence()：
    ...
    * 仕様説明  
      ...
      存在する場合はFUNCTION_TRUE、ない場合はFUNCTION_FALSEを返す。  
